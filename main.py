...
def get_engine(engine_type, definitions, spark, options):
...
def get_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--metric_db", help="Metric database", type=str, nargs="?", required=True)
    parser.add_argument("--sql_archive", help="SQL definition zip", type=str, nargs="?", required=True)
    parser.add_argument("--pymod_archive", help="SQL definition zip", type=str, nargs="?", required=False)
    ...
    parser.add_argument("--gcs_temp_bucket", type=str, nargs="?", required=False)
    parser.add_argument("--k8s_chdir", type=bool_parse, required=False, default="False")
...
def get_spark_session(keyfile=None, chdir=False, gcs_temp_bucket=None) -> SparkSession:
    ...
    if gcs_temp_bucket is not None:
        ...
        conf = conf.set("spark.hadoop.fs.gs.impl", "com.google.cloud.hadoop.fs.gcs.GoogleHadoopFileSystem")
        conf = conf.set("spark.hadoop.fs.AbstractFileSystem.gs.impl", "com.google.cloud.hadoop.fs.gcs.GoogleHadoopFS")
        conf = conf.set('spark.hadoop.fs.gs.auth.service.account.enable', 'true')

    if keyfile is not None:
        conf = conf.set('spark.hadoop.google.cloud.auth.service.account.json.keyfile', keyfile)
...
if __name__ == '__main__':
    ...
    spark = get_spark_session(args.keyfile, args.k8s_chdir, args.gcs_temp_bucket)
    definitions = load_definition(args.metric_db, args.sql_archive, args.pymod_archive)
